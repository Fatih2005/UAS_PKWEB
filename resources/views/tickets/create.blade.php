...
<head>
    ...
    <title>Buat Tiket Baru - PT. Ujug-Ujug</title>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-2xl shadow-soft border border-slate-100 p-6">
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <h1 class="text-xl font-bold text-slate-900">Buat Tiket Baru</h1>
                                <p class="text-sm text-slate-500">Isi detail permintaan atau masalah kamu dengan jelas</p>
                            </div>
                        </div>
                        ...
                        <form method="POST" action="{{ route('tickets.store') }}" enctype="multipart/form-data" class="space-y-5">
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="6" required
                                    class="w-full rounded-xl border-slate-300 bg-white shadow-sm focus:border-brand-500 focus:ring-brand-500/20 transition-all">{{ old('description') }}</textarea>
                                @error('description')
                                    <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                                ...
                                <div id="uploadArea" class="hidden mt-3">
                                    <div id="dropZone" class="drop-zone border-2 border-dashed border-slate-300 rounded-xl p-6 text-center cursor-pointer hover:border-brand-400 hover:bg-brand-50/40 transition-all">
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            <p class="text-sm text-slate-600">Seret file ke sini, atau <span class="font-medium text-brand-600">klik untuk pilih</span></p>
                            ...
                            <div class="flex items-center justify-end gap-3 pt-2">
                                <a href="{{ route('tickets.index') }}" class="px-4 py-2.5 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-100">Batal</a>
                                <button type="submit" class="btn-primary text-white px-5 py-2.5 rounded-xl text-sm font-medium">Buat Tiket</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <aside class="space-y-6">
                    <div class="bg-white rounded-2xl shadow-soft border border-slate-100 p-5">
                        <h3 class="text-sm font-semibold text-slate-900 mb-3">Panduan singkat</h3>
                        <ul class="space-y-3 text-sm text-slate-700">
                            <li class="flex gap-3">
                                <span class="mt-[0.15rem] flex-shrink-0 w-2.5 h-2.5 rounded-full bg-brand-100 border-2 border-brand-600"></span>
                                <div>
                                    <span class="font-medium">Judul jelas</span><br>
                                    <span class="text-slate-500">Contoh: "Error login setelah update password"</span>
                                </div>
                            </li>
                            <li class="flex gap-3">
                                <span class="mt-[0.15rem] flex-shrink-0 w-2.5 h-2.5 rounded-full bg-brand-100 border-2 border-brand-600"></span>
                                <div>
                                    <span class="font-medium">Deskripsi lengkap</span><br>
                                    <span class="text-slate-500">Sertakan langkah reproduction bila mungkin</span>
                                </div>
                            </li>
                            <li class="flex gap-3">
                                <span class="mt-[0.15rem] flex-shrink-0 w-2.5 h-2.5 rounded-full bg-brand-100 border-2 border-brand-600"></span>
                                <div>
                                    <span class="font-medium">Pilih prioritas</span><br>
                                    <span class="text-slate-500">Gunakan Critical hanya untuk insiden</span>
                                </div>
                            </li>
                        </ul>
                    </div>

                    <div class="bg-gradient-to-br from-brand-600 to-brand-800 rounded-2xl shadow-lg p-5 text-white">
                        <div class="flex items-start gap-3">
                            <div class="w-9 h-9 rounded-lg bg-white/15 flex items-center justify-center">
                                <i class="fa-solid fa-bolt text-sm"></i>
                            </div>
                            <div>
                                <h3 class="text-sm font-semibold">Tips prioritas</h3>
                                <p class="text-xs text-brand-100 mt-1 leading-relaxed">
                                    Critical = layanan down / keamanan.<br>
                                    High = gangguan signifikan.<br>
                                    Medium = fitur / umum.<br>
                                    Low = permintaan / enhancement.
                                </p>
                            </div>
                        </div>
                    </div>
